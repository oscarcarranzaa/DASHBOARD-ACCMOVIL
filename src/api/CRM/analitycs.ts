import axiosInstance from '@/lib/axiosClient'
import { isAxiosError } from 'axios'
import {
  AllFunnelMetricsSchema,
  AllResumeLeadsMetricsSchema,
  AllUserSellMetricsSchema,
  LeadsMetricsResponseSchema,
  ZAllFunnelMetrics,
  ZAllResumeLeadsMetrics,
  ZAllUserSellMetrics,
  ZLeadsMetricsResponse,
} from '@/types/crm/analitycs'

export type TParamsMetrics = {
  from?: string | null
  to?: string | null
  userId?: string | null
  funnelId?: string | null
}

function getParamsUrl({ from, to, userId, funnelId }: TParamsMetrics) {
  const paramsUrl = new URLSearchParams()
  const tz = Intl.DateTimeFormat().resolvedOptions().timeZone
  const entries = { from, to, userId, funnelId, tz }
  for (const key in entries) {
    const value = entries[key as keyof typeof entries]
    if (value) {
      paramsUrl.set(key, value)
    }
  }
  return paramsUrl
}
export async function getLeadsMetrics({
  from,
  to,
  userId,
  funnelId,
}: TParamsMetrics) {
  try {
    const paramsUrl = getParamsUrl({ from, to, userId, funnelId })

    const { data } = await axiosInstance.get<LeadsMetricsResponseSchema>(
      `/admin/lead/analitycs/metrics?${paramsUrl.toString()}`
    )
    const parseData = ZLeadsMetricsResponse.parse(data)
    return parseData
  } catch (error) {
    if (isAxiosError(error) && error.response) {
      throw new Error(error.response.data.response.msg, {
        cause: error.response.status,
      })
    } else {
      throw new Error('An unexpected error occurred while get leads metrics.')
    }
  }
}
export async function getResumeLeadsMetrics({
  from,
  to,
  userId,
  funnelId,
}: TParamsMetrics) {
  try {
    const paramsUrl = getParamsUrl({ from, to, userId, funnelId })

    const { data } = await axiosInstance.get<AllResumeLeadsMetricsSchema>(
      `/admin/lead/analitycs/rates?${paramsUrl.toString()}`
    )
    const parseData = ZAllResumeLeadsMetrics.parse(data)
    return parseData
  } catch (error) {
    if (isAxiosError(error) && error.response) {
      throw new Error(error.response.data.response.msg, {
        cause: error.response.status,
      })
    } else {
      throw new Error('An unexpected error occurred while get leads metrics.')
    }
  }
}

export async function getFunnelMetrics({
  from,
  to,
  userId,
  funnelId,
}: TParamsMetrics) {
  try {
    const paramsUrl = getParamsUrl({ from, to, userId, funnelId })

    const { data } = await axiosInstance.get<AllFunnelMetricsSchema>(
      `/admin/lead/analitycs/funnel?${paramsUrl.toString()}`
    )
    const parseData = ZAllFunnelMetrics.parse(data)
    return parseData
  } catch (error) {
    if (isAxiosError(error) && error.response) {
      throw new Error(error.response.data.response.msg, {
        cause: error.response.status,
      })
    } else {
      throw new Error('An unexpected error occurred while get funnel metrics.')
    }
  }
}

export async function getSellerMetrics({
  from,
  to,
  userId,
  funnelId,
}: TParamsMetrics) {
  try {
    const paramsUrl = getParamsUrl({ from, to, userId, funnelId })
    const { data } = await axiosInstance.get<AllUserSellMetricsSchema>(
      `/admin/lead/analitycs/seller?${paramsUrl.toString()}`
    )
    const parseData = ZAllUserSellMetrics.parse(data)
    return parseData
  } catch (error) {
    if (isAxiosError(error) && error.response) {
      throw new Error(error.response.data.response.msg, {
        cause: error.response.status,
      })
    } else {
      throw new Error('An unexpected error occurred while get seller metrics.')
    }
  }
}
