              ...
              return (
                <li key={index}>
                  ...
                  >
                    <div>
                      <button
                        className={`${style.activeVibrate} flex w-full flex-wrap mt-2 p-2 ${isOpen ? 'justify-center' : 'justify-between px-4 '} rounded-md ${isRouteActive ? 'bg-sky-100 text-sky-600 fill-sky-600 dark:bg-zinc-700 dark:text-white font-medium' : 'fill-black text-gray-800 dark:text-white hover:bg-zinc-200 dark:hover:bg-zinc-800'}`}
                        onClick={() => toggleMenu(index)}
                      >
                        <div className="flex">
                          <div
                            className={`stroke-black fill-black  dark:fill-zinc-300 dark:stroke-zinc-300 ${style.vibrate}`}
                          >
                            {menu.icon}
                          </div>
                          <p
